...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        b_a = batch[2].to(device)
        b_b = batch[3].to(device)
    
        model.zero_grad()

        output_a, output_b = model(b_input_ids, attention_mask=b_input_mask, labels_a=b_a, labels_b=b_b)
    ...
    for batch in test_dataloader:
        ...
        ground_truth_a.append(b_a.detach().cpu().numpy())
        
        pred_b = torch.argmax(logits_b, dim=1)      
        predictions_b.append(pred_b.detach().cpu().numpy())
        ground_truth_b.append(b_b.detach().cpu().numpy())
...
#saving as .csv file
values.to_csv('results/'+model_name+'_'+data+'_'+question+'_mtl.csv', mode='a', index=False)