...
from helpers.min_norm_solvers import MinNormSolver, gradient_normalizers
from helpers.data import EDAIC_meta
...
#preparing support, query & test data
if data == 'EDAIC':
    support_dataloader, query_dataloader, test_dataloader = EDAIC_meta(model_name, question, batch_size, max_seq_len, random_seed)
...
total_steps = len(support_dataloader) * epochs
...
def return_grad(not_task):
    task_grad = torch.empty(0).to(device)
    for name, param in model.named_parameters():
        if param.grad != None and not_task not in name:
            if param.requires_grad:
                task_grad = torch.cat((task_grad, param.grad.clone().detach().view(1,-1)[0]), 0)
    return task_grad

#initializing weights for losses
w_k = [1. for w in range(tasks)]
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(support_dataloader):
        ...
        task_grad_list_a.append(return_grad('taskb'))
        task_grad_list_b.append(return_grad('taska'))
        
        optimizer.step()

        scheduler.step()

    for i in range(tasks-1, 0, -1):
        task_grad_list_a[i] -= task_grad_list_a[i-1]
        task_grad_list_b[i] -= task_grad_list_b[i-1]

    for i in range(len(task_grad_list_a)):
        task_grad_list_a[i] /= (w_k[0] / tasks)
    for i in range(len(task_grad_list_b)):
        task_grad_list_b[i] /= (w_k[1] / tasks)
    
    loss_tr = total_tr_loss/len(support_dataloader)
    loss_tr_values.append(loss_tr)
    
    print('')
    print('Average training loss: {0:.2f}'.format(total_tr_loss/len(support_dataloader)))
    print('')
    ...
    for i, batch in enumerate(query_dataloader):
        ...
        qu_loss = loss_a + loss_b

        total_qu_loss += qu_loss.item()

        qu_loss.backward()

        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)

        query_grad_list_a.append(return_grad('taskb'))
        query_grad_list_b.append(return_grad('taska'))
        
    print('Average query loss: {0:.2f}'.format(total_qu_loss/len(query_dataloader)))
  
    w_list = {}
    w_list[0] = [torch.matmul(torch.stack(query_grad_list_a), torch.stack(task_grad_list_a).T)]
    w_list[1] = [torch.matmul(torch.stack(query_grad_list_b), torch.stack(task_grad_list_b).T)]
    
    scale, min_norm = MinNormSolver.find_min_norm_element([w_list[t] for t in range(tasks)])
    
    w_k[0] += (alpha * scale[0] * (sum(sum(w_list[0][0]))).item())
    w_k[1] += (alpha * scale[1] * (sum(sum(w_list[1][0]))).item())
...
#saving as .csv file
values.to_csv('results/'+model_name+'_'+data+'_'+question+'_meta.csv', mode='a', index=False)