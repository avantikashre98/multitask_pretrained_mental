...
train_ptsd = train.label_ptsd_44.values

input_ids, attention_masks = tokenizer_func(model_name, train_X)
    
train_inputs = torch.tensor(input_ids)
train_masks = torch.tensor(attention_masks)
train_ptsd_labels = torch.tensor(train_ptsd)

train_data = TensorDataset(train_inputs, train_masks, train_ptsd_labels)
...
test_ptsd = test.label_ptsd_44.values

input_ids, attention_masks = tokenizer_func(model_name, test_X)

test_inputs = torch.tensor(input_ids)
test_masks = torch.tensor(attention_masks)
test_ptsd_labels = torch.tensor(test_ptsd)

test_data = TensorDataset(test_inputs, test_masks, test_ptsd_labels)
...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        b_ptsd = batch[2].to(device)

    
        model.zero_grad()

        output = model(b_input_ids, attention_mask=b_input_mask, labels=b_ptsd)
        
        tr_loss = output[0]
        tr_loss_ptsd.append(tr_loss)
    ...
    for batch in test_dataloader:
        ...
        ground_truth.append(b_ptsd.detach().cpu().numpy())
    ...
    f1_ptsd = f1_score(flat_ground_truth, flat_predictions, average='binary')
    f1_ptsd_values.append(f1_ptsd)
    
    ba_ptsd = balanced_accuracy_score(flat_ground_truth, flat_predictions)
    ba_ptsd_values.append(ba_ptsd)
    
    auc_ptsd = roc_auc_score(flat_ground_truth, flat_predictions)
...
value_dict = {'model': model_name, 'epoch': range(epochs),
              ...
              'F1_ptsd': f1_ptsd_values,
              'BA_ptsd': ba_ptsd_values,
              'AUC_ptsd': auc_ptsd_values,
              'training_loss': loss_tr_values, 'testing_loss': loss_te_values,
              'training': 'STL'}
values = pd.DataFrame(data=value_dict)

#saving as .csv file
values.to_csv('results/'+model_name+'_EDAIC_'+question+'_ptsd.csv', mode='a', index=False)