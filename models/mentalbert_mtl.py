...
class MentalBERT_clf(nn.Module):
    def __init__(self, num_classes_a, num_classes_b):
        super(MentalBERT_clf, self).__init__()
        
        #shared layers
        self.mental = AutoModel.from_pretrained('mental/mental-bert-base-uncased', use_auth_token=True)
    ...
    def forward(self, input_id, attention_mask, labels_a=None, labels_b=None):
        outputs = self.mental(input_ids=input_id, attention_mask=attention_mask)
